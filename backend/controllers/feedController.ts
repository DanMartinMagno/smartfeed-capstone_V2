...
export const calculateFeed = (req: Request, res: Response) => {
  ...
  try {
    if (!selectedIngredients || !type || !numSwine) {
      throw new Error(
        "Missing required fields: selectedIngredients, type, and numSwine"
      );
    }
    ...
    console.error(
      "Error calculating feed formulation:",
      err.message,
      err.stack
    );
    res.status(500).json({
      error: "An error occurred while calculating the feed formulation.",
      details: err.message,
    });
  }
};
...
export const getNutrientAnalysis = (req: Request, res: Response) => {
  ...
  try {
    ...
    const recommendations = getNutrientRecommendations(
      type as "starter" | "grower" | "finisher"
    );
    ...
    res.status(500).json({
      error: "An error occurred while getting the nutrient analysis.",
      details: err.message,
    });
  }
};
