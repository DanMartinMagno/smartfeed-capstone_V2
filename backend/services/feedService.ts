...
const ingredients: Ingredient[] = [
  ...
  {
    name: 'Rice Bran',
    nutrients: { crudeProtein: 13.5, crudeFiber: 13, crudeFat: 2.9, calcium: 0.08, moisture: 11.3, phosphorus: 1.78, maxInclusionRate: -1 },
  },
];
...
const calculateNutrients = (selectedIngredients: Ingredient[], numSwine: number, feedType: 'starter' | 'grower' | 'finisher') => {
...
const feedFormulation = (selectedIngredients: string[], type: 'starter' | 'grower' | 'finisher', numSwine: number) => {
  ...
  const nutrientAnalysis = calculateNutrients(selectedIngredientObjects as Ingredient[], numSwine, type);
  ...
  const analysisResults = {
    ...
    analysis: {
      crudeProtein: perSwineNutrients.crudeProtein >= recommendations.crudeProtein ? 'Sufficient' : 'Deficient',
